...
void IntersectionUI::draw() {
	...
	shapenow->drawnow();   //draw the shape

    //Call HitRecord hr = intersect(pE1, dir);
	HitRecord hr= shapenow->intersect(pE1,dir);
...
void IntersectionUI::changeShape( ShapesUI::ShapeType type )
{
    ...
    case ShapesUI::SHAPE_SPHERE : 
		shapenow = new Sphere(t1);
		break;
    case ShapesUI::SHAPE_CYLINDER : 
		shapenow = new Cylinder(t1,t2);
		break;
    case ShapesUI::SHAPE_CONE : 
		shapenow = new Cone (t1,t2);
		break;
    case ShapesUI::SHAPE_CUBE : 
		shapenow = new Cube(t1);
		break;
	case ShapesUI::SHAPE_TORUS:
		shapenow = new Torus(t1,t2);
		break;
	case ShapesUI::SHAPE_CORKSCREW:
		shapenow = new Corkscrew(t1,t2);
		break;
...
void IntersectionUI::writeTest() const {
	...
	Cylinder cylinder = Cylinder(3,1);
	Cone cone = Cone(3,1);
	Cube cube = Cube(1);
	//Torus torus = Torus(3,3);
