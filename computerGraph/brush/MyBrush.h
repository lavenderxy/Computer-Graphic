...
private:
	...
	void constantDistr(const int radius);
	void LinearDistr(const int radius);
	void QuadraticDistr(const int radius);
	void GaussianDistr(const int radius);
	void SpecialDistr(const int radius);

	float create_mask(int x, int y);// create a mask by current pixel(x,y)
	void makemask(float val,int x , int y);// give the mask a specific value.
	//void makemask(float val);

	void drawLine1(int x0, int x1, int y0, int y1,Color colorLine);
    void drawLine2(int x0, int x1, int y0, int y1,Color colorLine);
	void colorin(int x0tmp, int x0tmp1, int x1tmp, int x1tmp1, int y0tmp, int y0tmp1, int y1tmp, int y1tmp1, Color colorLine);
	void circlemake(int R, int r);
};
...
//the pixels are found in row-major order, stored moving horizontally across a row, then the second row is
//stored, the third and so on.
inline
